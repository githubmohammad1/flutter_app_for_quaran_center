...
import '../../../providers/person_provider.dart'; // تأكد من المسار الصحيح
...
class _UnifiedLoginScreenState extends State<UnifiedLoginScreen> {
  ...
  final String _defaultTeacherPassword =
      defaultTeacherPassword; // كلمة مرور المعلم المبدئية
  ...
  Future<void> _login() async {
    ...
    if (isTeacher) {
      ...
      await personProvider.fetchAll(role: 'teacher', force: true);

      final teacher = personProvider.items.firstWhere(
        (p) =>
            p.firstName.trim() == _firstNameController.text.trim() &&
            p.lastName.trim() == _lastNameController.text.trim(),
        orElse: () => throw Exception('لم يتم العثور على المعلم'),
      );

      // حفظ بيانات المعلم مع رقم المعلم
      await prefs.setString('teacher_first_name', teacher.firstName);
      await prefs.setString('teacher_last_name', teacher.lastName);
      await prefs.setString('teacher_password', _passwordController.text);
      await prefs.setInt('teacher_id', teacher.id!); // حفظ رقم المعلم
      print(teacher.id);
      if (mounted) {
              ...
              child: Column(
                ...
                children: [
                  ...
                  TextButton(
                    ...
                    child: Text(
                      isTeacher ? 'تسجيل الدخول كطالب' : 'تسجيل الدخول كمعلم',
